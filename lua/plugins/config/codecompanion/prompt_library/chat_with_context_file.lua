...
return {
  interaction = "chat",
  description = "Chat with full tools",
  opts = {
    -- is_slash_cmd = true,
    alias = "context_file",
    index = 1,
  ...
  prompts = {
    {
      ...
      content = function()
        return
        "You have the tools @{full_stack_dev} @{web_search} @{mcp} so you can run terminal commands, CRUD files and edit current buffer. You can search the web to check new documents or searching for extra knowledge using the tool @{web_search}. Always use @{mcp} you can search newest documents of library using context7, always use sequentialthinking when response me, and let me know your thought each step."
      end,
    },
--     {
